...
class hg_furukawa_original(nn.Module):
    ...
    def init_weights(self):
        ...
        # Resolve absolute path to the model_1427.pth file relative to this file
        base_dir = os.path.dirname(os.path.abspath(__file__))
        model_path = os.path.join(base_dir, "model_1427.pth")

        if not os.path.exists(model_path):
            raise FileNotFoundError(f"Pretrained weights not found at: {model_path}")

        # Load pretrained model weights safely
        pretrained_model = model_1427.model_1427
        pretrained_model.load_state_dict(torch.load(model_path, map_location="cpu"))

        # Copy weights into the current model
        for src, dst in zip(pretrained_model.parameters(), self.parameters()):
