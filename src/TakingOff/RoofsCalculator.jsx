// frontend/src/components/RoofsCalculator.js
import React, { useState } from "react";
import axios from "axios";

function RoofsCalculator() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8000/calculate/roofs",
        {
          ...inputs,
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold mb-4">Roofs Quantity Takeoff</h2>
      <form
            ...
            <tbody>
              <tr>
                ...
                <td className="border border-gray-300 p-2">
                  {inputs.length} x{" "}
                  {(
                    parseFloat(inputs.width) /
                    2 /
                    Math.cos((parseFloat(inputs.pitch) * Math.PI) / 180)
                  ).toFixed(2)}
                </td>
                <td className="border border-gray-300 p-2">
                  {results.area_covering.toFixed(2)} m²
                </td>
              </tr>
              <tr>
                <td className="border border-gray-300 p-2">
                ...
                <td className="border border-gray-300 p-2">
                  {results.rafter_length.toFixed(2)} m
                </td>
              </tr>
            </tbody>
          </table>
...
export default RoofsCalculator;
