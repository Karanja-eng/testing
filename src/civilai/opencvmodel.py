...
YOLO_MODEL = None
if YOLO_OK and os.path.exists("src/civilai/best_casa1.pt"):
    try:
        YOLO_MODEL = YOLO("src/civilai/best_casa1.pt")
    except Exception:
        YOLO_MODEL = None
...
async def columns_endpoint(
    ...
    if img is None:
        return Response(
            content=error_image_bytes("Failed to read image"),
            media_type="image/png",
        )
    ...
    def _process(img):
        ...
        if not results:
            return npimg_to_png_bytes(np.ones_like(img) * 255)
    ...
    png = await run_blocking(_process, img)
    return Response(content=png, media_type="image/png")
...
async def detect_objects(
    file: UploadFile = File(...), confidence: float = 0.1, labels: list[str] = None
):
